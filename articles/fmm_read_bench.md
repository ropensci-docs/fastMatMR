...
This vignette demonstrates a benchmark comparing the `readMM` function
from the `Matrix` package against the `read_fmm` function from the
...
![plot of chunk fixed-sparse-read](fixed-sparse-read-1.png)
...
![plot of chunk varying-sparse-read](varying-sparse-read-1.png)

plot of chunk varying-sparse-read

## Conclusions

We see that though there are no statistically significant differences in
speed for small matrices, the `fastMatMR` package is significantly
faster for large matrices. This is because the `readMM` function from
the `Matrix` reads data into a triplet form, which gets slower for
larger matrices.
