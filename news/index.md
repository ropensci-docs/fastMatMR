...
- Transparent gzip support for all read/write functions via `.mtx.gz`
  file extension
  ([\#28](https://github.com/ropensci/fastMatMR/issues/28))
- Add
  [`fmm_to_spam()`](https://docs.ropensci.org/fastMatMR/reference/fmm_to_spam.md)
  and
  [`spam_to_fmm()`](https://docs.ropensci.org/fastMatMR/reference/spam_to_fmm.md)
  for reading/writing `spam` sparse matrices
  ([\#2](https://github.com/ropensci/fastMatMR/issues/2))
- Add
  [`fmm_to_SparseM()`](https://docs.ropensci.org/fastMatMR/reference/fmm_to_SparseM.md)
  and
  [`SparseM_to_fmm()`](https://docs.ropensci.org/fastMatMR/reference/SparseM_to_fmm.md)
  for reading/writing `SparseM` `matrix.csr` objects
  ([\#1](https://github.com/ropensci/fastMatMR/issues/1))
- [`write_fmm()`](https://docs.ropensci.org/fastMatMR/reference/write_fmm.md)
  dispatcher now handles `spam` and `matrix.csr` inputs
...
- Move vendored fast_matrix_market headers from `inst/include/` to
  `src/` to reduce installed package size (headers not needed at
  runtime)
...
- Handle paths with `~`
  ([\#24](https://github.com/ropensci/fastMatMR/issues/24))
- Integrated support to handle integers in input files.
  ([\#25](https://github.com/ropensci/fastMatMR/issues/25))
- Added support for handling `.mtx.gz` file formats.
  ([\#28](https://github.com/ropensci/fastMatMR/issues/28))
...
- Improved error messages distinguishing between sparse and non-sparse
  matrices. ([\#26](https://github.com/ropensci/fastMatMR/issues/26))
- Enhanced error messages for `sparse_to_fmm` function.
  ([\#27](https://github.com/ropensci/fastMatMR/issues/27))

### Breaking changes

- Streamlined function naming for better clarity.
  ([\#29](https://github.com/ropensci/fastMatMR/issues/29))
...
- Support reading `R` numeric vectors
  ([`is.vector()`](https://rdrr.io/r/base/vector.html))
- Support reading of `R`
  [`is.matrix()`](https://rdrr.io/r/base/matrix.html) numerical matrices
...
- A full multi-OS set of tests conforming to CRAN submission
  requirements and ROpenSci is present.
  ([\#12](https://github.com/ropensci/fastMatMR/issues/12))
- Support writing `R` numeric vectors
  ([`is.vector()`](https://rdrr.io/r/base/vector.html))
- Support writing of `R`
  [`is.matrix()`](https://rdrr.io/r/base/matrix.html) numerical matrices
